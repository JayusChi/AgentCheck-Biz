...
新增本次验收恢复索引、可配置的隔离业务历史、平台健康检查与控制器自行停止、逐文件摘要校验和新环境交付入口；将 D34 预览/预算边界及 D35 交付边界加入 CI。复用既有网络、恢复验证器和业务检查器，没有改变业务通过标准。224 项核心测试、3 个网络/4 个恢复、真实 A/B/C 及重复缺陷门禁、新源码环境与浏览器验收通过。新请求 0；独立第三方试用仍待反馈。见 [D35 记录](docs/v2/d35.md)。
