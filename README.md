...
仓库统一使用 `main` 维护和演示。原 CI 验证分支与候选交付分支已合入 `main`，阶段成果保留在提交历史中。

| 分支 / 标签 | 当前用途 | 演示选择 |
| --- | --- | --- |
| `main` | 默认分支，包含业务结果验证、候选交付、最新文档与自动化验收 | **日常使用与演示入口** |
...
后续改动统一更新到 `main`，固定发布版本使用标签保留。合并分支仅简化代码管理，不改变候选版的验收范围与独立试用状态。
...
```powershell
git clone --branch main https://github.com/JayusChi/AgentCheck-Biz.git AgentCheck-Biz-demo
Set-Location AgentCheck-Biz-demo
```

如需精确复现 RC1，将克隆命令中的 `--branch main` 改为 `--branch v2.0.0-rc.1`；Git 会处于 detached HEAD（固定提交）状态，可直接安装和运行。也可从 [RC1 发布页](https://github.com/JayusChi/AgentCheck-Biz/releases/tag/v2.0.0-rc.1) 下载源码包。后续命令均在项目根目录执行。
